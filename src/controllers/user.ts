...
export async function putProfile(req: JWTRequest, res: Response, next: NextFunction) {
    try {
        const { id } = getAuthUser(req);
        const { name, phone_num, birth_date, location_ids /*, profile_url*/ } = req.body as {
            name: string;
            phone_num: string;
            birth_date: string;
            location_ids: number[];
        ...
        if (phone_num !== '') user.phone = phone_num;
...
export async function putPassword(req: JWTRequest, res: Response, next: NextFunction) {
    try {
        ...
        const updatedResult = await userRepository.update({ id }, { password_hash: password_hash });
