...
export function createAuthMiddleware({ secret, userRepository, logger = console, isFindEmail }: AuthMiddlewareOptions) {
    ...
    return async (req: JWTRequest, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            if (isFindEmail) user = await userRepository.findOneBy({ email: verifyResult.email });
            else user = await userRepository.findOneBy({ id: verifyResult.id });

            if (!user) {
                if (isFindEmail) next(generateError(1017));
                else next(generateError(1014));
                return;
            }
